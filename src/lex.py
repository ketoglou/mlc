...
class Lex:
    ...
    def __init__(self, file_name,error_handler):
        ...
        try:
            ...
            self.error_handler.file_lex_error_handler(file_lex_error_types.FileNotFound, file_name)
    ...
    def start_read(self):
        ...
        while current_state != State.FINAL:
            ...
            if c == "" and current_state != State.INITIAL:
                self.error_handler.file_lex_error_handler(file_lex_error_types.UnexpectedEnd)
            elif c == "" and current_state == State.INITIAL:
                return (None, None)  # RETURN None when EOF
            elif c not in Symbols.ALL:
                self.error_handler.file_lex_error_handler(file_lex_error_types.UnexpectedChar,self.file_index)
