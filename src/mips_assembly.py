...
class mips_assembly:
    ...
    def __init__(self,file_name,list_of_programs):
        #Create assembly file
        self.file_mips = open(file_name[0:-4] + ".asm","w")
        #Open for read idermediate file
        self.file_int = open(file_name[0:-4] + ".int","r")

        #list_of_programs contain at potsition 0 the main
        #followed by a child program of main with all its childs and their childs(if have any)
        #followed by a child program of main with all its childs and their childs(if have any)
        #etc
        self.list_of_programs = list_of_programs
        self.main_start_quad = self.list_of_programs[0].starting_quad
    ...
    #Make assembly code and save at $t0 the address(in stack) of the variable that does not belong to the current program
    #return 0 if variable is in main(global), return 1 if is local variable of a parent program
    #return 2 if is "in" argument of a parent program, return 3 if is "inout" argument of a parent program
    def gnvlcode(self,variable,program_pos):
        current_program = self.list_of_programs[program_pos]
        ...
        if current_program.nesting_level > 1:
            nl_parent = current_program.nesting_level #This variable is for check only parents and not their brothers
            for i in range(program_pos-1,0,-1):
                parent_program = self.list_of_programs[i]
                if nl_parent != parent_program.nesting_level:
                    if parent_program.nesting_level < current_program.nesting_level:
                        counter += 1
                        if variable in parent_program.variables:
                            offset = 12 + 4*len(parent_program.arguments) + parent_program.variables.index(variable)
                            break
                        for j in range(0,len(parent_program.arguments)):
                            arg = parent_program.arguments[j]
                            if variable == arg[1]:
                                offset = 12 + 4*j
                                in_or_inout = arg[0]
                                break
                    nl_parent = parent_program.nesting_level
                #Break if we check and the last parent
                if parent_program.nesting_level == 1:
                    break
    ...
    def loadvr(self,var,reg,program_pos):
        program = self.list_of_programs[program_pos]
        ...
        len_vars = len(program.variables)
        len_args = len(program.arguments)
        len_temp = program.temporary_variables
        is_local = False
        ...
        if self.represents_int(var):
            ...
            is_local = True
        #elif local variable
        elif var in program.variables:
            var_pos = program.variables.index(var)
        ...
        if is_local == False:
            where_belongs = self.gnvlcode(var,program_pos)

            #if where_belongs == 0 then it belongs to main(global)
            if where_belongs == 0:
                main = self.list_of_programs[0]
                offset = 12 + 4*main.variables.index(var)
                self.add_command("lw "+reg+",-"+str(offset)+"($s0)")
            #else if where_belongs == 1 then it is a local variable of a parent program
            #or if where_belongs == 2 then it is a "in" argument of a parent program
            elif where_belongs == 1 or where_belongs == 2:
                self.add_command("lw "+reg+",($t0)")
            #else(where_belongs == 3) then it is a "inout" argument of a parent program
            else:
    ...
    def storerv(self,reg,var,program_pos):
        program = self.list_of_programs[program_pos]
    ...
    def translate_int_to_ass(self):
        ...
        while line:
            ...
            if quad[0] == "begin_block":
                ...
                current_function = self.list_of_programs[current_function_pos]
                if in_main:
                ...
                if function_call_offset == 12:
                    ...
                    function_call_framelength = self.list_of_programs[function_call_pos].frame_length
                ...
                if quad[2] == "CV":
                    ...
                    #if var or cv in current program then load its address in $t0
                    if arg_type == 0:
                        self.add_command("addi $t0,$sp,-"+str(offset))
                    #elif ref in current program then load its parent address in $t0
                    elif arg_type == 1:
                        self.add_command("lw $t0,-"+str(offset)+"($sp)")
                    #elif var or cv or ref in parent program
                    elif arg_type == 3:
                        ...
                        if where_belongs == 3: #ref in another parent function
                            ...
                            offset = 12 + 4*self.list_of_programs[0].variables.index(quad[1])
                ...
                if self.list_of_programs[function_call_pos].nesting_level == self.list_of_programs[current_function_pos]:
                    self.add_command("lw $t0,-4($sp)")
                ...
                label_jal = "L" + str(self.list_of_programs[function_call_pos].starting_quad)
...
#Helping functions
    ...
    def find_function_pos_sq(self,starting_quad):
        for i in range(0,len(self.list_of_programs)):
            ...
            if program.starting_quad == starting_quad:
                return i
    ...
    #Finds the position in list_of_programs of the called function
    #pos : current program position
    #quad : the current quad
    #return program (that called by current program) position in the list_of_programs
    def find_function_pos(self,quad,pos):
        ...
        if pos == 0:
            for i in range(1,len(self.list_of_programs)):
                function = self.list_of_programs[i]
                if name == function.program_name and type_called == function.program_type and len(arguments) == len(function.arguments):
        ...
        #1)Its own childen functions(all functions following it with nesting level of program + 1 until nesting level < nesting level of program)
        #2)Can call it self(recursive)
        #3)Can can all parent and brother functions(all functions in front of it until nesting level == 1 and after that only the program in front of it with nesting level == 1)
        else:

            current_function = self.list_of_programs[pos]
            ...
            if function_pos == -1:
                #If we are here it means that this function have no children or the children programs checked without find anything
                #Next thing is to check if the function call it self
                if name == current_function.program_name and type_called == current_function.program_type and len(arguments) == len(current_function.arguments):
    ...
    def find_variable_in_parent(self,var,parent_pos):
        current_program = self.list_of_programs[parent_pos]
        
        offset = 12
        len_args = len(current_program.arguments)
        for i in range(0,len_args):
            par_arg = current_program.arguments[i]
            if par_arg[1] == var:
                offset += 4*i
                if par_arg[0] == "CV":
                    return 0,offset
        ...
        len_vars = len(current_program.variables)
        for i in range(0,len_vars):
            par_var = current_program.variables[i]
            if par_var == var:
